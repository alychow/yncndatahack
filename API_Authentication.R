...
transactions <- mutate(transactions, corp = str_detect(transactions$product, "Career Fair"))
transactions <- mutate(transactions, LP = str_detect(transactions$product, "LP"))

tmp <- filter(transactions, !(startup | corp | LP))
tmp2 <- filter(transactions, startup)
